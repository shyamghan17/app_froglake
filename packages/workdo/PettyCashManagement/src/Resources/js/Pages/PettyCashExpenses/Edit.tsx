...
import { EditPettyCashExpenseProps, EditPettyCashExpenseFormData } from './types';
import { usePage } from '@inertiajs/react';
import { useEffect, useState } from 'react';
import axios from 'axios';

export default function EditPettyCashExpense({ pettycashexpense, onSuccess }: EditPettyCashExpenseProps) {
    const { pettycashrequests, users } = usePage<any>().props;

    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<EditPettyCashExpenseFormData>({
        request_id: pettycashexpense.request_id ?? '',
        type: pettycashexpense.type?.toString() ?? '0',
        amount: pettycashexpense.amount ?? '',
        remarks: pettycashexpense.remarks ?? '',
        status: pettycashexpense.status?.toString() ?? '0',
        approved_at: pettycashexpense.approved_at ?? '',
        approved_by: pettycashexpense.approved_by ?? '',
        created_by: pettycashexpense.created_by ?? '',
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('petty-cash-management.petty-cash-expenses.update', pettycashexpense.id), {
            onSuccess: () => {
                onSuccess();
            }
        });

        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-6">
                    ...
                    {promotion.approved_by && (
                        <div className="bg-green-50 p-4 rounded-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-lg">{promotion.approved_by.name}</h3>
                                    <p className="text-sm text-green-600">{t('Approved By')}</p>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="bg-white p-6 rounded-lg border">
                    <h4 className="font-semibold mb-4 text-gray-800 flex items-center justify-between">
                        {t('Promotion Details')}
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadge(promotion.status || 'pending')}`}>
                            ...
                            {promotion.document && (
                                <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="font-semibold text-gray-800">{t('Promotion Document')}</p>
                                    </div>
                                    <Button 
                                        variant="outline" 
                                        size="sm"
                                        asChild
                                        className="text-blue-600 hover:text-blue-700 border-blue-200 h-8"
                                    >
                                        <a href={getImagePath(promotion.document)} download>

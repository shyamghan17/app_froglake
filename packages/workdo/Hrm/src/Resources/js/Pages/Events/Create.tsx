...
export default function Create({ onSuccess }: CreateEventProps) {
    const { eventtypes, users, departments,auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {eventtypes?.length === 0 && auth?.user?.permissions?.includes('create-event-types') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create event type here.')} <button type="button" onClick={() => router.get(route('hrm.event-types.index'))} className="text-blue-600 hover:underline">{t('Create event type')}</button>
                        </p>
                    )}
                </div>
                ...
                <div>
                    ...
                    {departments?.length === 0 && auth?.user?.permissions?.includes('create-departments') && (
                        <p className="text-xs text-gray-500 mt-1">

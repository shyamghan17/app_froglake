...
class PromotionController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-promotions')) {
            $promotions = Promotion::with([
                ...
                ->when(request('previous_branch_id'), fn($q) => $q->where('previous_branch_id', request('previous_branch_id')))
                ->when(request('current_branch_id'), fn($q) => $q->where('current_branch_id', request('current_branch_id')))
    ...
    public function updateStatus(Promotion $promotion)
    {
        if (Auth::user()->can('manage-promotions-status')) {
            ...
            return redirect()->back()->with('success', __('The promotion status has been updated successfully.'));
        } else {

...
class TerminationController extends Controller
{
    ...
    public function store(StoreTerminationRequest $request)
    {
        if (Auth::user()->can('create-terminations')) {
            ...
            // Check if employee already has an approved termination
            $existingTermination = Termination::where('employee_id', $validated['employee_id'])
                ->where('status', 'approved')
                ->first();
            if ($existingTermination) {
                return redirect()->route('hrm.terminations.index')
            ...
            $termination->document = basename($validated['document']);
    ...
    public function update(UpdateTerminationRequest $request, Termination $termination)
    {
        if (Auth::user()->can('edit-terminations')) {
            ...
            // Check if employee already has an approved termination (excluding current one)
            $existingTermination = Termination::where('employee_id', $validated['employee_id'])
                ->where('status', 'approved')
    ...
    public function updateStatus(Request $request, Termination $termination)
    {
        if (Auth::user()->can('manage-termination-status')) {
            ...
            return redirect()->back()->with('success', __('The termination status has been updated successfully.'));
        } else {

...
class LeaveTypeController extends Controller
{
    ...
    public function store(StoreLeaveTypeRequest $request)
    {
        if(Auth::user()->can('create-leave-types')){
            ...
            return redirect()->route('hrm.leave-types.index')->with('success', __('The leave type has been created successfully.'));
        }
        else{
    ...
    public function update(UpdateLeaveTypeRequest $request, LeaveType $leavetype)
    {
        if(Auth::user()->can('edit-leave-types')){
            ...
            return redirect()->back()->with('success', __('The leave type details are updated successfully.'));
        }
        else{
            return redirect()->route('hrm.leave-types.index')->with('error', __('Permission denied'));
        }
    ...
    public function destroy(LeaveType $leavetype)
    {
        if(Auth::user()->can('delete-leave-types')){
            ...
            return redirect()->back()->with('success', __('The leave type has been deleted.'));
        }
        else{
            return redirect()->route('hrm.leave-types.index')->with('error', __('Permission denied'));
        }

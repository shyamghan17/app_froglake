...
class ProjectApiController extends Controller
{
    ...
    private function updateProject(Request $request)
    {
        if (Auth::user()->can('edit-project')) {
            ...
            $validated = $validator->validated();
            $project = Project::where('created_by', creatorId())->where('id', $request->project_id)->first();
    ...
    public function fileDelete($id)
    {
        try {
            if (!Module_is_active('Taskly')) {
                return $this->errorResponse('Taskly Module Not Active!');
            }
            $file = ProjectFile::where('id', $id)
                ->first();
            if (!$file) {
                return $this->errorResponse('File Not Found', '', 404);
            }
            $file->delete();
            return $this->successResponse('', 'The files has been deleted.');
        } catch (\Exception $e) {
            return $this->errorResponse('Something went wrong');
        }
    }

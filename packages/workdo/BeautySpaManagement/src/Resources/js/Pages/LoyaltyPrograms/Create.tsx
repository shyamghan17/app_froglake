...
import { CreateBeautyLoyaltyProgramProps, CreateBeautyLoyaltyProgramFormData } from './types';
import { usePage } from '@inertiajs/react';
import { useEffect, useState } from 'react';
import axios from 'axios';

export default function Create({ onSuccess }: CreateBeautyLoyaltyProgramProps) {
    const { } = usePage<any>().props;

    const { t } = useTranslation();
    const { data, setData, post, processing, errors } = useForm<CreateBeautyLoyaltyProgramFormData>({
        customer_name: '',
        points_earned: '',
        points_redeemed: '',
        last_updated: '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('beauty-spa-management.beauty-loyalty-programs.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Create Loyalty Programs')}</DialogTitle>
            </DialogHeader>

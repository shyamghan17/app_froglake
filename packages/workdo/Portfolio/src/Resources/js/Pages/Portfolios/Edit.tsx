...
import { EditPortfolioFormData } from "./types";

export default function Edit() {
    const { portfolio, portfoliocategories } = usePage<any>().props;
    const [activeTab, setActiveTab] = useState("developer");


    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<EditPortfolioFormData>({
        ...
        // Work Details
        category_id: portfolio.category_id?.toString() || "",
        ...
        show_overview: portfolio.show_overview !== undefined ? portfolio.show_overview : true,

        // Gallery
        images: Array.isArray(portfolio.images) ? portfolio.images : (portfolio.images ? [portfolio.images] : []),
        video_link: portfolio.video_link || "",
        show_gallery: portfolio.show_gallery !== undefined ? portfolio.show_gallery : true,

        // Contact Section
        contact_heading: portfolio.contact_heading || "",
        contact_message: portfolio.contact_message || "",
        show_contact: portfolio.show_contact !== undefined ? portfolio.show_contact : true,

        // Custom Sections
        custom_sections: portfolio.custom_sections || [{ title: "", content: "" }],
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route("portfolio.portfolios.update", portfolio.id));
    };
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                ...
                { label: t("Edit Portfolio") },
            ]}
            pageTitle={t("Edit Portfolio")}
        >
            <Head title={t("Edit Portfolio")} />

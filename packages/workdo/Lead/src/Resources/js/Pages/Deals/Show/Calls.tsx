...
export default function Calls({ deal, onRegisterAddHandler }: CallsProps) {
    ...
    const handleCallSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingCall) {
            router.put(route('deal.calls.update', editingCall.id), callForm, {
                onSuccess: () => {
            ...
            router.post(route('deal.calls.store'), {
                deal_id: deal.id,
                ...callForm
            }, {
                onSuccess: () => {
    ...
    const confirmCallDelete = () => {
        if (callDeleteState.callId) {
            router.delete(route('deal.calls.destroy', callDeleteState.callId));
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <TimePicker
                                    ...
                                    onChange={(time) => setCallForm({...callForm, duration: time})}
                            ...
                            <Select value={callForm.assignee} onValueChange={(value) => setCallForm({...callForm, assignee: value})}>
                                ...
                                <SelectContent>
                                    {deal.user_deals?.map((userDeal: any) => (
                                        <SelectItem key={userDeal.user?.id} value={userDeal.user?.id?.toString() || ''}>
                                            {userDeal.user?.name || ''}
                                        </SelectItem>
                                    ))}
                                </SelectContent>

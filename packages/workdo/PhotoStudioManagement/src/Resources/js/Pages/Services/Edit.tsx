...
import { EditServiceProps, ServiceFormData } from './types';

export default function Edit({ service, onClose, serviceCategories, cameraKits }: EditServiceProps) {
    const { t } = useTranslation();
    const { auth } = usePage().props as any;

    const { data, setData, put, processing, errors } = useForm<ServiceFormData>({
        name: service.name,
        service_category_ids: Array.isArray(service.service_category_ids) ? service.service_category_ids.map(String) : [],
        description: service.description || '',
        image: service.image || '',
        price: service.price?.toString() || '',
        status: Boolean(service.status),
        camera_kit_ids: Array.isArray(service.camera_kit_ids) ? service.camera_kit_ids.map(String) : [],
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('photo-studio-management.services.update', service.id), {
            onSuccess: () => onClose(),
        });
    };

...
import { CreateRepairOrderRequestProps, CreateRepairOrderRequestFormData } from './types';
import { usePage } from '@inertiajs/react';

export default function Create({ onSuccess }: CreateRepairOrderRequestProps) {
    const { repairtechnicians, repairstatuses } = usePage<any>().props;

    const { t } = useTranslation();
    const { data, setData, post, processing, errors } = useForm<CreateRepairOrderRequestFormData>({
        product_name: '',
        product_quantity: '',
        customer_name: '',
        customer_email: '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('repair-management-system.repair-order-requests.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Create Order Request')}</DialogTitle>
            </DialogHeader>

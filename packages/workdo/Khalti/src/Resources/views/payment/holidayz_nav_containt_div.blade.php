<div class="price-summery-card">
    <div class="price-sm-card-top">
        ...
        <form method="POST" action="{{ route('hotel.pay.with.khalti',$slug) }}"
            ...
            <div class="form-group text-right">
                <button type="submit"
                    class="btn" id="pay_with_khalti">{{ __('Pay Now') }}</button>
            </div>
        </form>
    </div>
</div>
...
<script>
        ...
        "eventHandler": {
            onSuccess (payload) {
                if(payload.status==200) {
                    ...
                    $.ajax({
                        url: "{{ route('booking.get.khalti.status',$slug) }}",
                        method: 'POST',
                        data :{
                            'payload':payload,
    ...
    var btn = document.getElementsByClassName("pay_with_khalti")[0];
</script>
...
<script>
  ...
  function get_khalti_status(){
      ...
      $.ajax({
          url: '{{ route('hotel.pay.with.khalti',$slug) }}',
          method: 'POST',
          data : {
                'coupon': $('.coupon_code').val(),
                'firstname': $("input[name='firstname']").val(),

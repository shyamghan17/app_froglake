...
import { ItemsIndexProps, Item, ItemFilters } from './types';
...
export default function Index() {
    ...
    const itemTypes = ['bookings', 'product', 'service', 'part'];
    ...
    const { deleteState, openDeleteDialog, closeDeleteDialog, confirmDelete } = useDeleteHandler({
        routeName: 'bookings.items.destroy',
        defaultMessage: t('Are you sure you want to delete this item?')
    });

    const handleFilter = () => {
        router.get(route('bookings.items.index'), {...filters, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('bookings.items.index'), {...filters, per_page: perPage, sort: field, direction, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('bookings.items.index'), {per_page: perPage, view: viewMode});
    };
    ...
    const tableColumns = [
        {
            ...
            render: (value: string) => {
                ...
                const imageSrc = getImagePath(value);
                return isImage ? (
        ...
        ...(auth.user?.permissions?.some((p: string) => ['edit-booking-items', 'delete-booking-items'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, item: Item) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('edit-booking-items') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.visit(route('bookings.items.edit', item.id))} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                        ...
                        {auth.user?.permissions?.includes('delete-booking-items') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
            ...
            <AuthenticatedLayout
                ...
                pageActions={
                    <div className="flex gap-2">
                        {auth.user?.permissions?.includes('create-booking-items') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button size="sm" onClick={() => router.visit(route('bookings.items.create'))}>
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <ListGridToggle
                                ...
                                routeName="bookings.items.index"
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            createPermission="create-booking-items"
                                            onCreateClick={() => router.visit(route('bookings.items.create'))}
                                    ...
                                    {items.data.map((item) => (
                                        <Card key={item.id} className="border border-gray-200">
                                            <div className="p-4">
                                                ...
                                                {item.image ? (() => {
                                                    const isImage = /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(item.image);
                                                    ...
                                                    return isImage ? (
                                                        ...
                                                        <div className="w-12 h-12 bg-gray-100 rounded-md border flex items-center justify-center cursor-pointer hover:bg-gray-200 transition-colors" onClick={() => {
                                                            const link = document.createElement('a');
                                                            link.href = imageSrc;
                                                            link.download = item.image.split('/').pop() || 'file';
                                                            link.click();
                                                        }}>
                                                            <Download className="w-6 h-6 text-gray-600" />
                                                        </div>
                                                ...
                                                <div className="space-y-3 mb-3">
                                                    {item.sku && (
                                                        <div>
                                                            <p className="text-xs font-medium text-gray-600 mb-2">{t('SKU')}</p>
                                                            <p className="text-xs text-gray-900 truncate">{item.sku}</p>
                                                        </div>
                                                    )}
                                                <div className="flex justify-between">
                                                    <span className="text-sm text-gray-600">{t('Type')}</span>
                                                    <span className="px-2 py-1 rounded-full text-sm bg-blue-100 text-blue-800 capitalize">{item.type ? t(item.type) : '-'}</span>
                                                </div>
                                                {item.sale_price && (
                                                    <div className="flex justify-between">
                                                ...
                                                <TooltipProvider>
                                                    {auth.user?.permissions?.includes('edit-booking-items') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => router.visit(route('bookings.items.edit', item.id))} className="h-9 w-9 p-0 text-blue-600 hover:text-blue-700 hover:bg-blue-50">
                                                                    <Edit className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>

...
import { Package, Item, ExtraService, PackageFormData } from './types';
...
export default function Edit({ open, onOpenChange, packageData, items, extraServices }: EditProps) {
    const { t } = useTranslation();
    const { data, setData, put, processing, errors, reset } = useForm<PackageFormData>({
        name: '',
        item_id: '',
        services: [],
        delivery_time: '',
    ...
    useEffect(() => {
        if (packageData) {
            let parsedServices = [];
            try {
                parsedServices = packageData.services ? JSON.parse(packageData.services) : [];
            } catch (e) {
                parsedServices = [];
            }
            setData({
                name: packageData.name,
                item_id: packageData.item_id.toString(),
                services: parsedServices,
                delivery_time: packageData.delivery_time,
                delivery_period: packageData.delivery_period,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (packageData) {
            put(route('bookings.packages.update', packageData.id), {
                onSuccess: () => {
                    reset();
                    onOpenChange(false);
                }

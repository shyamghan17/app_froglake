...
export default function Create({ onSuccess }: CreateLeaveApplicationProps) {
    ...
    const calendarFields = useFormFields('getCalendarSyncFields', data, setData, errors, 'create', t, 'Rotas');
    ...
    useEffect(() => {
        if (data.employee_id && data.leave_type_id) {
            axios.get(route('rotas.leave-balance', [data.employee_id, data.leave_type_id]))
                .then(response => {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('rotas.leave-applications.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle>{t('Create Leave Application')}</DialogTitle>
                <DialogDescription>
                    {t('Submit a new leave application for approval.')}
                </DialogDescription>
            </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Select value={data.leave_type_id?.toString() || ''} onValueChange={(value) => setData('leave_type_id', value)} required>
                            ...
                            <SelectContent searchable={true} required>
                ...
                {leaveBalance && (
                    <div className="p-3 bg-blue-50 rounded-lg">
                        <div className="text-sm space-y-1">
                            ...
                            <p>{t('Used')}: {leaveBalance.used_leaves} {t('days')} ( {leaveBalance.approved_leaves} {t('Approved')} + {leaveBalance.pending_leaves} {t('Pending')} )</p>

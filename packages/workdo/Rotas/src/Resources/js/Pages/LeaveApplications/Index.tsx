...
import CreateLeaveApplication from './Create';
import EditLeaveApplication from './Edit';
import ViewLeaveApplication from './View';
...
export default function Index() {
    ...
    const { deleteState, openDeleteDialog, closeDeleteDialog, confirmDelete } = useDeleteHandler({
        routeName: 'rotas.leave-applications.destroy',
        defaultMessage: t('Are you sure you want to delete this leaveapplication?')
    });

    const handleFilter = () => {
        router.get(route('rotas.leave-applications.index'), { ...filters, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode }, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('rotas.leave-applications.index'), { ...filters, per_page: perPage, sort: field, direction, view: viewMode }, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('rotas.leave-applications.index'), { per_page: perPage, view: viewMode });
    };
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            sortable: false,
            render: (value: any, row: any) => (
                <span className={`px-2 py-1 rounded-full text-sm ${row.leave_type?.is_paid
        ...
        ...(auth.user?.permissions?.some((p: string) => ['manage-rotas-leave-status', 'view-rotas-leave-applications', 'edit-rotas-leave-applications', 'delete-rotas-leave-applications'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, leaveapplication: LeaveApplication) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        {leaveapplication.attachment && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <a href={getImagePath(leaveapplication.attachment)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                        ...
                        {auth.user?.permissions?.includes('manage-rotas-leave-status') && leaveapplication.status === 'pending' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                        ...
                        {auth.user?.permissions?.includes('view-rotas-leave-applications') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                        ...
                        {auth.user?.permissions?.includes('edit-rotas-leave-applications') && leaveapplication.status === 'pending' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                        ...
                        {auth.user?.permissions?.includes('delete-rotas-leave-applications') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
    ...
    return (
        <AuthenticatedLayout
            ...
            pageActions={
                <TooltipProvider>
                    {auth.user?.permissions?.includes('create-rotas-leave-applications') && (
                        <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
        ...
        >
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <ListGridToggle
                                ...
                                routeName="rotas.leave-applications.index"
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-blue-50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
                            {auth.user?.permissions?.includes('manage-rotas-employees') && (
                                <div>
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            createPermission="create-rotas-leave-applications"
                            ...
                            {leaveapplications?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {leaveapplications?.data?.map((leaveapplication) => (
                                        <Card key={leaveapplication.id} className="p-6 hover:shadow-md transition-shadow">
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-primary/10 rounded-lg">
                                                        <FileTextIcon className="h-5 w-5 text-primary" />
                                                    </div>
                                                    <h3 className="font-semibold text-lg">{leaveapplication.employee?.name || 'Unknown Employee'}</h3>
                                                </div>
                                            </div>
                                            <div className="space-y-3 mb-6">
                                                <div className="text-sm">
                                                    <p className="text-muted-foreground mb-1">{t('Leave Type')}</p>
                                                    <div className="flex items-center gap-2">
                                                        <div
                                                            className="w-3 h-3 rounded-full"
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div className="text-sm">
                                                        ...
                                                        <p className="font-medium">{leaveapplication.start_date ? formatDate(leaveapplication.start_date) : '-'}</p>
                                                    </div>
                                                    <div className="text-sm">
                                                        <p className="text-muted-foreground mb-1">{t('End Date')}</p>
                                                        <p className="font-medium">{leaveapplication.end_date ? formatDate(leaveapplication.end_date) : '-'}</p>
                                                    </div>
                                                </div>
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div className="text-sm">
                                                        <p className="text-muted-foreground mb-1">{t('Total Days')}</p>
                                                        <p className="font-medium">{leaveapplication.total_days || '-'}</p>
                                                    </div>
                                                    <div className="text-sm">
                                                        <p className="text-muted-foreground mb-1">{t('Is Paid')}</p>
                                                        <span className={`text-sm px-2 py-1 rounded-full w-fit ${leaveapplication.leave_type?.is_paid
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t">
                                                <span className={`px-2 py-1 rounded-full text-sm ${leaveapplication.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                {modalState.mode === 'add' && (
                    <CreateLeaveApplication onSuccess={closeModal} />
                )}
            ...
            <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
                {viewingItem && <ViewLeaveApplication leaveapplication={viewingItem} />}
            </Dialog>

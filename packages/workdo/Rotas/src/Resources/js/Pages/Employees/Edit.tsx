...
export default function Edit() {
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (documentsWithFiles.length > 0) {
            ...
            router.post(route('rotas.employees.update', employee.id), formData, {
                forceFormData: true,
                preserveState: false,
            });
        } else {
            // If no documents with files, send regular form data
            put(route('rotas.employees.update', employee.id), {
                data: employeeData
            });
        }
    };
                    ...
                    <form onSubmit={submit} className="pt-5">
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                            ...
                            <TabsContent value="employment" className="space-y-6 mt-6">
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    <div>
                                        <Label htmlFor="user_id" required={!employee.user_id}>{t('User')}</Label>
                                        {employee.user_id ? (
                                            <>
                                                <Input
                                                    id="user_id"
                                                ...
                                                />
                                                <p className="text-sm text-muted-foreground mt-1">{t('User cannot be changed after assignment.')}</p>
                                            </>
                                        ) : (
                                            <>
                            ...
                            <TabsContent value="documents" className="space-y-6 mt-6">
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        onClick={() => {
                                                                            if (confirm(t('Are you sure you want to delete this document?'))) {
                                                                                router.delete(route('rotas.employee-documents.destroy', [employee.id, doc.id]));
                                                                            }
                                                                        }}
                                ...
                                {data.documents.map((document: any, index: number) => (
                                    <Card key={index} className="p-4 border-dashed">
                                        ...
                                        {(index > 0 || existingDocuments?.length > 0) && (

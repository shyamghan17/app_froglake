...
import { Plus, Edit as EditIcon, Trash2, Eye, Users as UsersIcon, UserIcon, Download, FileImage } from "lucide-react";
...
export default function Index() {
    ...
    const { deleteState, openDeleteDialog, closeDeleteDialog, confirmDelete } = useDeleteHandler({
        routeName: 'rotas.employees.destroy',
        defaultMessage: t('Are you sure you want to delete this employee?')
    });

    const handleFilter = () => {
        router.get(route('rotas.employees.index'), { ...filters, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode }, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('rotas.employees.index'), { ...filters, per_page: perPage, sort: field, direction, view: viewMode }, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('rotas.employees.index'), { per_page: perPage, view: viewMode });
    };
    ...
    const tableColumns = [
        {
            ...
            render: (value: string, employee: Employee) =>
                auth.user?.permissions?.includes('view-rotas-employees') ? (
                    <span className="text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => router.get(route('rotas.employees.show', employee.id))}>{value}</span>
                ) : (
                    <span>{value}</span>
                )
        },
        ...
        ...(auth.user?.permissions?.some((p: string) => ['edit-rotas-employees', 'delete-rotas-employees'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, employee: Employee) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-rotas-employees') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('rotas.employees.show', employee.id))} className="h-8 w-8 p-0 text-green-600 hover:text-green-700">
                        ...
                        {auth.user?.permissions?.includes('edit-rotas-employees') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.visit(route('rotas.employees.edit', employee.id))} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                        ...
                        {auth.user?.permissions?.includes('delete-rotas-employees') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
            ...
            pageActions={
                <div className="flex gap-2">
                    <TooltipProvider>
                        ...
                        {auth.user?.permissions?.includes('create-rotas-employees') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button size="sm" onClick={() => router.visit(route('rotas.employees.create'))}>
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <ListGridToggle
                                ...
                                routeName="rotas.employees.index"
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            createPermission="create-rotas-employees"
                                            onCreateClick={() => router.visit(route('rotas.employees.create'))}
                                            ...
                                            <div className="p-4 bg-gradient-to-r from-primary/5 to-transparent border-b flex-shrink-0">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-semibold text-sm text-gray-900">{employee.user?.name || '-'}</h3>
                                                        <p className="text-xs font-medium text-primary">{employee.employee_id}</p>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <TooltipProvider>
                                                    {auth.user?.permissions?.includes('view-rotas-employees') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => router.get(route('rotas.employees.show', employee.id))} className="h-9 w-9 p-0 text-green-600 hover:text-green-700">
                                                                    <Eye className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                    ...
                                                    {auth.user?.permissions?.includes('edit-rotas-employees') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => router.visit(route('rotas.employees.edit', employee.id))} className="h-9 w-9 p-0 text-blue-600 hover:text-blue-700">
                                                                    <EditIcon className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>

...
export default function Index() {
    ...
    const { deleteState, openDeleteDialog, closeDeleteDialog, confirmDelete } = useDeleteHandler({
        routeName: 'rotas.announcements.destroy',
        defaultMessage: t('Are you sure you want to delete this announcement?')
    });

    const handleFilter = () => {
        router.get(route('rotas.announcements.index'), { ...filters, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode }, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('rotas.announcements.index'), { ...filters, per_page: perPage, sort: field, direction, view: viewMode }, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('rotas.announcements.index'), { per_page: perPage, view: viewMode });
    };
    ...
    const tableColumns = [
        ...
        ...(auth.user?.permissions?.some((p: string) => ['manage-rotas-announcements-status', 'view-rotas-announcements', 'edit-rotas-announcements', 'delete-rotas-announcements'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, announcement: Announcement) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('manage-rotas-announcements-status') && (
                        ...
                        {auth.user?.permissions?.includes('view-rotas-announcements') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                        ...
                        {auth.user?.permissions?.includes('edit-rotas-announcements') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                        ...
                        {auth.user?.permissions?.includes('delete-rotas-announcements') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
    ...
    return (
        <AuthenticatedLayout
            ...
            pageActions={
                <TooltipProvider>
                    {auth.user?.permissions?.includes('create-rotas-announcements') && (
                        <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
        ...
        >
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <ListGridToggle
                                ...
                                routeName="rotas.announcements.index"
                            ...
                            <div className="relative">
                                ...
                                />
                                {(() => {
                                    const activeFilters = [filters.announcement_category_id, filters.priority, filters.status].filter(f => f !== '' && f !== null && f !== undefined).length;
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-blue-50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">{t('Announcement Category')}</label>
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            hasFilters={!!(filters.title || filters.description || filters.announcement_category_id || filters.priority || filters.status)}
                                            onClearFilters={clearFilters}
                                            createPermission="create-rotas-announcements"
                                            ...
                                            <div className="p-4 flex flex-col h-full">
                                                <div className="flex items-center justify-between mb-3">
                                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                                        ...
                                                        <h3 className="font-semibold text-sm break-word">{announcement.title}</h3>
                                                    </div>
                                                </div>
                                                <div className="space-y-2 mb-3 flex-1">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div className="text-xs">
                                                            <p className="text-muted-foreground mb-1">{t('Approved By')}</p>
                                                            <p className="font-medium">{announcement.approved_by?.name || '-'}</p>
                                                        </div>
                                                        <div className="text-xs">
                                                            <p className="text-muted-foreground mb-1">{t('Priority')}</p>
                                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                                ...
                                                                {t(announcement.priority?.charAt(0).toUpperCase() + announcement.priority?.slice(1) || 'Unknown')}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between pt-3 border-t">
                                                    <span className={`px-2 py-1 rounded-full text-sm ${
                                                        ...
                                                        announcement.status === 'draft' ? 'bg-blue-100 text-blue-800' :
                                                        'bg-gray-100 text-gray-800'
                                                    }`}>
                                                        {t(announcement.status?.charAt(0).toUpperCase() + announcement.status?.slice(1) || 'Unknown')}
                                                    </span>
                                                    {auth.user?.permissions?.some((p: string) => ['manage-rotas-announcements-status', 'view-rotas-announcements', 'edit-rotas-announcements', 'delete-rotas-announcements'].includes(p)) && (
                                                        <div className="flex gap-1">
                                                            <TooltipProvider>
                                                                {auth.user?.permissions?.includes('manage-rotas-announcements-status') && (
                                                                    <Tooltip delayDuration={300}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => setStatusModalState({ isOpen: true, announcement })} className="h-7 w-7 p-0 text-purple-600 hover:text-purple-700 hover:bg-purple-50">
                                                                ...
                                                                {auth.user?.permissions?.includes('view-rotas-announcements') && (
                                                                    <Tooltip delayDuration={300}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => setViewingItem(announcement)} className="h-7 w-7 p-0 text-green-600 hover:text-green-700 hover:bg-green-50">
                                                                ...
                                                                {auth.user?.permissions?.includes('edit-rotas-announcements') && (
                                                                    <Tooltip delayDuration={300}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => openModal('edit', announcement)} className="h-7 w-7 p-0 text-blue-600 hover:text-blue-700 hover:bg-blue-50">
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                {modalState.mode === 'add' && (
                    <CreateAnnouncement onSuccess={closeModal} />
                )}

...
import { CreateDesignationProps, DesignationFormData } from './types';

export default function Create({ onSuccess, branches, departments }: CreateDesignationProps) {
    const { t } = useTranslation();
    const { data, setData, post, processing, errors } = useForm<DesignationFormData>({
        designation_name: '',
        branch_id: '',
        department_id: '',
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('rotas.designations.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Create Designation')}</DialogTitle>
                <DialogDescription>
                    {t('Create a new designation within a department and branch.')}
                </DialogDescription>
            </DialogHeader>

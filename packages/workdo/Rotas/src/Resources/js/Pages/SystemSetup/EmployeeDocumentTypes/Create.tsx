...
export default function Create({ onSuccess }: CreateEmployeeDocumentTypeProps) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('rotas.employee.document.types.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Create Document Type')}</DialogTitle>
                <DialogDescription>
                    {t('Create a new employee document type for your organization.')}
                </DialogDescription>
            </DialogHeader>

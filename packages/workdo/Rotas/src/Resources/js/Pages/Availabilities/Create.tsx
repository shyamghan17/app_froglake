...
import { CreateAvailabilityProps, CreateAvailabilityFormData } from './types';
...
export default function Create({ onSuccess }: CreateAvailabilityProps) {
    const { employees, currentUserEmployee, isNotEmpType, startTime, endTime, companyAllSetting } = usePage<any>().props;
    ...
    const { data, setData, post, processing, errors } = useForm<CreateAvailabilityFormData>({
        employee_id: isNotEmpType ? '' : (currentUserEmployee?.id?.toString() || ''),
        name: '',
        start_date: '',
        end_date: '',
        availability: [],
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('rotas.availabilities.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle>{t('Create Availability')}</DialogTitle>
            </DialogHeader>
                            ...
                            <div style={{ height: '400px' }}>
                                <FullCalendar
                                    ...
                                    slotMinTime={(() => {
                                        ...
                                        return selectedEmployee?.shift?.start_time || startTime || '00:00:00';
                                    })()}
                                    slotMaxTime={(() => {
                                        const selectedEmployee = employees?.find((emp: any) => emp.id.toString() === data.employee_id);
                                        ...
                                        return shiftEndTime || endTime || '24:00:00';
                                    })()}

...
export default function Edit() {
    ...
    const { item, taxes, categories, units, warehouses } = usePage<any>().props;
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: item.name || '',
        sku: item.sku || '',
        tax_ids: Array.isArray(item.tax_ids) ? item.tax_ids.map(id => id?.toString() || '') : [],
        ...
        warehouse_id: item.warehouse_stocks?.[0]?.warehouse_id?.toString() || '',
        type: 'eyewear',
        product_type: item.product_type || '',
        brand_name: item.brand_name || '',
        prescription_detail: item.prescription_detail || '',
        numbering_status: item.numbering_status || 'numbering',
        customization_details: item.customization_details || ''
    });

    const descriptionAI = useFormFields('aiField', data, setData, errors, 'edit', 'description', 'Short Description', 'opticalandeyecarecenter', 'eyewearitem');
    ...
    const validatePricingTab = () => {
        return data.sale_price.toString().trim() !== '' &&
            data.purchase_price.toString().trim() !== '' &&
            data.unit !== '' &&
            data.quantity.toString().trim() !== '';
    };
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('optical-and-eye-care-center.eyewear-items.update', { eyewearitem: item.id }));
    };
            ...
            <AuthenticatedLayout
                ...
                pageTitle={
                    <div className="flex items-center justify-between">
                        <span>{t('Edit Eyewear Item')}</span>
                    </div>
                }
            >
                <Head title={t('Edit Eyewear Item')} />
                                ...
                                <TabsContent value="warehouse" className="space-y-6 mt-6">
                                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                        <div>
                                            ...
                                            <Select value={data.warehouse_id?.toString() || ''} onValueChange={(value) => setData('warehouse_id', value)} required>
                                                <SelectTrigger>

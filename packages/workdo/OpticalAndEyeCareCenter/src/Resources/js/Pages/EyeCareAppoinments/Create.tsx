...
import { CreateEyeCareAppoinmentProps, CreateEyeCareAppoinmentFormData } from './types';
import { usePage } from '@inertiajs/react';
import { useEffect } from 'react';

export default function Create({ onSuccess }: CreateEyeCareAppoinmentProps) {
    const { eyepatients, opticaldoctors } = usePage<any>().props;
    const { t } = useTranslation();
    const { data, setData, post, processing, errors } = useForm<CreateEyeCareAppoinmentFormData>({
        patient_id: '',
        doctor_name: '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('optical-and-eye-care-center.eye-care-appoinments.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Create Eye Care Appointment')}</DialogTitle>
            </DialogHeader>
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    <Select value={data.doctor_name} onValueChange={(value) => setData('doctor_name', value)}>
                        <SelectTrigger>

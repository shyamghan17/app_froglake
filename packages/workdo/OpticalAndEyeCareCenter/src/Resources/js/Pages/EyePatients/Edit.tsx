...
import { EditEyePatientProps, EditEyePatientFormData } from './types';

export default function EditEyePatient({ eyepatient, onSuccess }: EditEyePatientProps) {
    const { doctors } = usePage<any>().props;
    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<EditEyePatientFormData>(eyepatient);



    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('optical-and-eye-care-center.eye-patients.update', eyepatient.id), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Edit Eye Patient')}</DialogTitle>
            </DialogHeader>
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    <Select value={data.preferred_doctor?.toString() || ''} onValueChange={(value) => setData('preferred_doctor', value)}>
                        <SelectTrigger>
